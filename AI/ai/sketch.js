  ...
  for (let i = 0; i < poses.length; i++) {
    ...
    for (let j = 0; j < poses[i].pose.keypoints.length; j++) {
      ...
      if (keypoint.score > 0.2) {
        ...
        ctx.beginPath();
        ctx.arc(keypoint.position.x+10, keypoint.position.y-10, 5, 0, 2 * Math.PI);
        ctx.stroke();
        if(i==0&&j==0||i==0&&j==9||i==0&&j==10)
...
function drawSkeleton() {
  ...
  for (let i = 0; i < poses.length; i++) {
    ...
    for (let j = 0; j < poses[i].skeleton.length; j++) {
      ...
      ctx.beginPath();
      ctx.moveTo(partA.position.x+10, partA.position.y-10);
      ctx.lineTo(partB.position.x+10, partB.position.y-10);
      ctx.stroke();
    }
  }
}
