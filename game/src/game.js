...
var MyGame = function() {
	...
	var FINAL_MAX_SPEED = 7000; // 结束最大速度
	var SIDE_ACCEL = 500;				// 侧面加速度
	...
	function init(){
		...
		presentGroup.position.x = ATUtil.randomRange(-MyConfig.FLOOR_WIDTH/2, MyConfig.FLOOR_WIDTH/2);
		presentGroup.position.z = ATUtil.randomRange(-MyConfig.FLOOR_DEPTH/2, MyConfig.FLOOR_DEPTH/2);
	...
	function setFloorHeight(){
		...
		// 变换宝石
		presentGroup.position.z += MyConfig.MOVE_STEP;
		if (presentGroup.position.z + moverGroup.position.z > MyConfig.FLOOR_DEPTH/2){
			presentGroup.collided = false;
			presentGroup.position.z	-= MyConfig.FLOOR_DEPTH;
			//重新随机化x位置
